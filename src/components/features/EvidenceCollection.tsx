...
function getCategoryLabel(category: EvidenceCategory): string {
  return (
    EVIDENCE_CATEGORIES.find((c) => c.value === category)?.label || category
  );
}
...
const MetaDialog: React.FC<MetaDialogProps> = ({
  title,
  initialCaption = "",
  busy,
  onCancel,
...
const AudioRecorder: React.FC<AudioRecorderProps> = ({
  ...
  const startRecording = async () => {
    try {
      ...
      setError(
        "No se pudo acceder al micrófono. Verifica los permisos del dispositivo.",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .catch(() => {
        if (!cancelled)
          setError(
            "No se pudo acceder a la cámara/micrófono. Verifica los permisos.",
          );
      });
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between text-white text-sm">
          <div
            className={cn(
              ...
              isRecording ? "bg-red-600 animate-pulse" : "bg-black/50",
            )}
          >
...
const TextNoteCapture: React.FC<TextNoteCaptureProps> = ({
  busy,
  onCapture,
  onCancel,
}) => {
...
export const EvidenceCollection: React.FC<EvidenceCollectionProps> = ({
  ...
  const [filterCategory, setFilterCategory] = useState<EvidenceCategory | null>(
    null,
  );
  ...
  const evidence: EvidenceItem[] = React.useMemo(() => {
    void entriesVersion;
    return (
      store.getEntriesByIncident(incidentId) as StoredDocumentationEntry[]
    ).map((entry) => {
      const category =
        entry.category && isEvidenceCategory(entry.category)
          ? entry.category
          : "otros";
      const type = (
        ["photo", "video", "audio", "text"].includes(entry.type)
          ? entry.type
          : "text"
      ) as EvidenceType;
      return {
    ...
    async (params: {
      ...
      try {
        const entryData: Omit<
          StoredDocumentationEntry,
          "id" | "hash" | "chainOfCustody" | "timestamp"
        > = {
          incidentId,
  ...
  const buildItemReport = (
    item: EvidenceItem,
  ): string => `EVIDENCIA - PROTOCOLO CDMX
===========================
            ...
            <div className="space-y-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg mb-4">
              <div className="grid grid-cols-2 gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setFilterCategory(
                      (e.target.value as EvidenceCategory) || null,
                    )
                  }
                ...
                <select
                  value={filterType || ""}
                  onChange={(e) =>
                    setFilterType((e.target.value as EvidenceType) || null)
                  }
                  className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm"
                >
      ...
      <main className="max-w-lg mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-4 gap-2 mb-6">
          {[
            ...
            <div
              key={i}
              className="bg-white dark:bg-gray-900 p-3 rounded-lg text-center"
            >
            ...
            {displayEvidence.map((item) => (
              <div
                ...
                className={cn(
                  ...
                  viewMode === "grid"
                    ? "aspect-square"
                    : "flex gap-4 p-3 bg-white dark:bg-gray-900",
      ...
      {mode === "camera" && !pendingMedia && (
        <CameraCapture
          onCapture={(imageData, location) => {
            void handleMediaCaptured("photo", imageData, location);
          }}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60">
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Esta acción no se puede deshacer. El elemento se eliminará del
              registro cifrado.
            </p>
            <div className="flex gap-2">
              <Button
          ...
          <div className="flex items-center justify-between p-4 text-white">
            <div>
              ...
              <p className="text-sm text-gray-400">
                ...
                {new Date(viewingItem.timestamp).toLocaleString("es-MX")}
              </p>
            </div>
            <div className="flex gap-2">
              <button
              ...
              <button
                onClick={() => setDeleteCandidate(viewingItem.id)}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="text-white">
                  {formatFileSize(viewingItem.fileSize)}
                </div>
              </div>
              {viewingItem.location && (
                <div>
                  ...
                  <div className="text-white">
                    {viewingItem.location.latitude.toFixed(6)},{" "}
                    {viewingItem.location.longitude.toFixed(6)}
                  </div>
                </div>
              )}
              <div>
