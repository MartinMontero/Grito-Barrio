...
export function LegalResourceDetailPage({
  resource,
  onBack,
}: LegalResourceDetailPageProps) {
    ...
    .map((paragraph, index) => {
      if (paragraph.startsWith("**") && paragraph.endsWith("**")) {
        ...
        return (
          <h4
            key={index}
            className="font-semibold text-base mt-4 mb-2 text-primary"
          >
            {paragraph.replace(/\*\*/g, "")}
          </h4>
        );
        ...
        return (
          <li
            key={index}
            className="ml-4 text-sm text-muted-foreground leading-relaxed"
          >
    ...
    <div className="space-y-4 pb-20">
      ...
      <div className="px-4">
        ...
        <p className="text-xs text-muted-foreground mb-6">
          ...
          {new Date(resource.lastUpdated).toLocaleDateString("es-MX")}
        </p>
        ...
        <div className="mt-6 bg-amber-50 border border-amber-200 rounded-lg p-4">
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            ...
            No constituye asesoría legal profesional. Para casos específicos, te
            recomendamos consultar con un abogado o defensor especializado en
            derecho de vivienda.
          </p>
        </div>
