...
export function HomePage({ onEmergencyPress, onNavigate }: HomePageProps) {
  ...
  const activeIncidents = incidents.filter((i) =>
    ["detected", "verified", "responding", "contained"].includes(i.status),
  );
    ...
    <div className="space-y-4 pb-20">
      ...
      <section className="px-4 pt-2">
        <h1 className="text-2xl font-bold text-foreground">
          {currentUser
            ? `Hola, ${currentUser.pseudonym}`
            : "Bienvenido a Protocolo CDMX"}
        </h1>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                <h2 className="font-semibold text-destructive">
                  ¿Estás en riesgo de desalojo?
                </h2>
                <p className="text-sm text-muted-foreground mt-1">
                  Activa el protocolo de emergencia inmediatamente para obtener
                  ayuda
                </p>
                <Button
                  variant="destructive"
      ...
      <section className="px-4 pb-4">
        <div className="bg-muted rounded-lg p-4">
          <p className="text-sm text-muted-foreground text-center">
            <strong>Nota importante:</strong> Esta aplicación proporciona
            información de apoyo. No sustituye el asesoramiento legal
            profesional. En caso de emergencia, llama al 911.
          </p>
        </div>
      </section>
    </div>
