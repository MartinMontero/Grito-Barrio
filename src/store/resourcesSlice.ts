...
import type { SafePoint, SupplyItem, ContactTree, UserRole, SafePointType, SupplyCategory } from '@/types'
import {
  getCurrentTimestamp,
  updateInArray,
  findById,
...
export interface ResourcesSlice {
  ...
  getNearbySafePoints: (coordinates: { latitude: number; longitude: number }, radiusKm: number) => SafePoint[]
  verifySafePoint: (id: string) => void
  toggleSafePointAvailability: (id: string) => void

  // Contact Actions
  addContact: (contact: Omit<ContactTree, 'priority'> & { priority: 1 | 2 | 3 | 4 | 5 }) => void
...
> = persistToIndexedDB<ResourcesSlice>('protocolo-resources', false)(
  (set, get) => ({
     ...
     * Add a new contact
     */
    addContact: (contact: Omit<ContactTree, 'priority'> & { priority: 1 | 2 | 3 | 4 | 5 }) => {
      set(state => ({
