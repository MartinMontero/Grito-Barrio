...
export const createIncidentSlice: StateCreator<
  ...
  [],
  IncidentSlice
> = persistToIndexedDB<IncidentSlice>(
  "protocolo-incidents",
  true,
)((set, get) => ({
  ...
  closeIncident: (
    ...
    set({
      ...
      activeIncidentId:
        state.activeIncidentId === id ? null : state.activeIncidentId,
    });
  },
  ...
  assignTeamMember: (incidentId: string, member: TeamMember) => {
    set((state) => {
      ...
      const exists = incident.team.some(
        (m) => m.pseudonym === member.pseudonym,
      );
      if (exists) {
      ...
      return {
        incidents: state.incidents.map((inc) =>
          inc.id === incidentId ? { ...inc, team: [...inc.team, member] } : inc,
        ),
      };
    });
  },
