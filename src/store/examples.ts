...
export async function exampleCreateIncident() {
  ...
  const alertData: AlertData = {
    ...
    alertSource: "hotline",
    description:
      "Desalojo ilegal en proceso. Fuerza pública presente sin orden judicial.",
    threatLevel: "high",
    occupantsAtRisk: 8,
    minorsPresent: true,
...
export async function exampleCompleteWorkflow() {
  ...
  const incidentId = store.createIncident({
    ...
    description:
      "Desalojo de familias en edificio habitacional. Sin orden judicial.",
  ...
  store.closeIncident(
    incidentId,
    "Resuelto mediante mediación exitosa",
    "successful",
  );
