...
const initialDocumentationState: Omit<
  DocumentationSlice,
...
export const createDocumentationSlice: StateCreator<
  ...
  [],
  DocumentationSlice
> = persistToIndexedDB<DocumentationSlice>(
  "protocolo-documentation",
  true,
)((set, get) => ({
  ...
  verifyIntegrity: async (entryId: string): Promise<boolean> => {
    ...
    const hasValidCustody =
      entry.chainOfCustody.length > 0 &&
