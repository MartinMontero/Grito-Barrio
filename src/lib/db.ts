...
class IndexedDBWrapper {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        this.db.onversionchange = () => {
          this.db?.close()
          this.db = null
          console.warn('Database version changed in another tab. Please reload.')
        }
   ...
   */
  async get<T>(storeName: StoreName, id: string | number): Promise<DBOperationResult<T>> {
   ...
   */
  async getAll<T>(storeName: StoreName, options?: QueryOptions): Promise<DBOperationResult<T[]>> {
    return this.withRetry(async () => {
      await this.init()
  ...
  async getByIndex<T>(
    ...
    return this.withRetry(async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onerror = () => {
          reject(new Error(`Failed to get items by index: ${request.error?.message}`))
        }
      })
    })
  }
   ...
   */
  async putBatch<T>(storeName: StoreName, items: T[]): Promise<DBOperationResult<number>> {
    return this.withRetry(async () => {
      await this.init()
      ...
      return new Promise((resolve, reject) => {
        ...
        transaction.oncomplete = () => {
          resolve({ 
            success: failed === 0, 
            data: completed,
            error: failed > 0 ? new Error(`${failed} items failed to save`) : undefined
          })
        }
   ...
   */
  async delete(storeName: StoreName, id: string | number): Promise<DBOperationResult<void>> {
    return this.withRetry(async () => {
      await this.init()
   ...
   */
  async deleteBatch(storeName: StoreName, ids: (string | number)[]): Promise<DBOperationResult<number>> {
    return this.withRetry(async () => {
      await this.init()
      ...
      return new Promise((resolve, reject) => {
        ...
        transaction.oncomplete = () => {
          resolve({ 
            success: failed === 0, 
            data: completed,
            error: failed > 0 ? new Error(`${failed} items failed to delete`) : undefined
          })
        }
   ...
   */
  async count(storeName: StoreName, keyRange?: IDBKeyRange): Promise<DBOperationResult<number>> {
    return this.withRetry(async () => {
      await this.init()
   ...
   */
  async getStats(): Promise<DBOperationResult<{
...
export async function requestPersistentStorage(): Promise<boolean> {
  if (navigator.storage && navigator.storage.persist) {
    try {
      ...
      console.log(`[DB] Persistent storage: ${isPersistent ? 'granted' : 'denied'}`)
      return isPersistent
    } catch (error) {
