...
function addHeader(
  ...
  doc.text(
    `Creado: ${new Date(formData.createdAt).toLocaleString("es-MX")}`,
    20,
    y + 25,
  );
  ...
  doc.text(
    `Estado: ${statusLabels[formData.status] || formData.status}`,
    120,
    y + 25,
  );
...
export async function generateIncidentReportPDF(
  ...
  doc.text(
    `Fecha: ${new Date(incidentData.timestamp).toLocaleString("es-MX")}`,
    20,
    58,
  );
