...
class MigrationManager {
  ...
  async init(): Promise<void> {
    ...
    if (this.state.currentVersion < CURRENT_DATA_VERSION) {
      console.log(`[Migration] Auto-migrating from v${this.state.currentVersion} to v${CURRENT_DATA_VERSION}`)
      await this.migrate()
    }
  }
  ...
  async migrate(): Promise<boolean> {
    const pendingMigrations = getMigrations().filter(
      m => m.version > this.state.currentVersion && m.version <= CURRENT_DATA_VERSION
    )
  ...
  async validateIntegrity(): Promise<DataIntegrityReport> {
    ...
    const stores: StoreName[] = ['incidents', 'documentation', 'checklists', 'users']
        ...
        if (result.success && result.data) {
          ...
          for (const item of result.data) {
            ...
            if (validation.valid) {
              ...
              report.errors.push(`[${storeName}] ${validation.errors.join(', ')}`)
            }
          }
        }
      } catch (error) {
    ...
    for (const [storeName, storeReport] of Object.entries(report.stores)) {
      if (storeReport.invalid > 0) {
        try {
          ...
          if (result.success && result.data) {
            for (const item of result.data) {
              const validation = this.validateItem(storeName as StoreName, item)
  ...
  private async createBackup(): Promise<boolean> {
    try {
      ...
      if (result.success) {
        ...
        storage.session.set(`migration_backup_v${this.state.currentVersion}`, backup)
        return true
      }
   ...
   */
  private validateItem(storeName: StoreName, item: unknown): { valid: boolean; errors: string[] } {
    const errors: string[] = []
...
export async function fixDataIntegrity(): Promise<{ fixed: number; errors: string[] }> {
