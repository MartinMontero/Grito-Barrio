...
class SyncQueue {
   ...
   */
  add(action: Omit<SyncAction, 'id' | 'timestamp' | 'retryCount' | 'status'>): string {
    ...
    const fullAction: SyncAction = {
      ...
      retryCount: 0,
      status: 'pending',
      priority: action.priority || DEFAULT_CONFIG.priorityRules[action.type] || 5
    }
...
class SyncEngine {
  ...
  async init(): Promise<void> {
    ...
    const persistedState = storage.local.get<Partial<SyncState>>(SYNC_STATE_KEY)
    if (persistedState) {
   ...
   */
  queueUpdate(store: StoreName, id: string, data: unknown, priority?: number): string {
    return this.queue.add({
      type: 'UPDATE',
      store,
      data: { ...data, id },
  ...
  async sync(): Promise<SyncResult[]> {
    ...
    console.log(`[Sync] Completed. Success: ${results.filter(r => r.success).length}/${results.length}`)
  ...
  startAutoSync(): void {
    ...
    this.syncInterval = window.setInterval(() => {
      if (this.state.isOnline && !this.state.isSyncing && this.queue.getPendingCount() > 0) {
        this.sync()
      }
