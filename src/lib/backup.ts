...
class BackupManager {
  ...
  async createBackup(
    ...
    try {
      ...
      console.log(`[Backup] Created: ${metadata.id} (${this.formatSize(metadata.size)})`)
   ...
   */
  async createSelectiveBackup(options: ExportOptions): Promise<BackupData | null> {
    try {
      const data: Record<string, unknown[]> = {}
      const stores = options.stores || ['incidents', 'documentation', 'checklists', 'users']
  ...
  async exportToFile(
    ...
    try {
      ...
      const blob = new Blob([data], {
        type: backup.metadata.encrypted ? 'application/encrypted' : 'application/json'
      })

      const url = URL.createObjectURL(blob)
      const finalFilename = filename || `protocolo_cdmx_backup_${backup.metadata.timestamp.split('T')[0]}.json`
  ...
  async exportForSharing(
    ...
    try {
      ...
      return this.exportToFile(backup, `protocolo_cdmx_incident_${incidentIds[0]}.json`)
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Export failed'
  ...
  scheduleBackups(intervalHours: number = 24): void {
    ...
    if (!lastBackup || now - new Date(lastBackup).getTime() > interval) {
      this.createBackup('scheduled', `Scheduled backup every ${intervalHours}h`)
