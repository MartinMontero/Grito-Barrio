...
describe("checklistSlice (single source of truth)", () => {
  ...
  it("toggling an item off reverts completion and clears metadata", () => {
    ...
    expect(
      useProtocoloStore.getState().checklists[INCIDENT_A][0].completed,
    ).toBe(true);
  ...
  it("keeps incident checklists isolated from each other", () => {
    ...
    expect(
      useProtocoloStore.getState().getProgress(INCIDENT_A),
    ).toBeGreaterThan(0);
  ...
  it("phase progress reflects completions within a single phase", () => {
    ...
    const phaseItems = useProtocoloStore
      .getState()
      .getItemsByPhase(INCIDENT_A, "0-5min");
    expect(phaseItems.length).toBeGreaterThan(0);

    // Complete every item in the first phase.
    phaseItems.forEach((item) =>
      store.toggleItem(INCIDENT_A, item.id, "tester"),
    );

    expect(
      useProtocoloStore.getState().getPhaseProgress(INCIDENT_A, "0-5min"),
    ).toBe(100);
  });
});
