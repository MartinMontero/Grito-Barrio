...
describe("vault: lifecycle", () => {
  ...
  it("lock drops the key; unlock with right pass restores it; wrong pass fails", async () => {
    ...
    expect(await vault.unlock("master-pass-123")).toEqual({
      success: true,
      duress: false,
    });
  });
...
describe("storage: fail-closed at-rest persistence", () => {
  it("encrypts at rest when unlocked and decrypts back", async () => {
    ...
    const raw = await db.get<{ data: string; encrypted: boolean }>(
      "settings" as never,
      "incidents-x",
    );
    ...
    expect(await getData<{ secret: string }>("incidents-x")).toEqual({
      secret: "eviction notes",
      n: 42,
    });
  });
  ...
  it("REFUSES to persist sensitive data while locked (no plaintext leak)", async () => {
    ...
    vault.lock();
    await expect(
      storeData("incidents-y", { secret: "do not leak" }, true),
    ).rejects.toBeTruthy();
  });
  ...
  it("returns null for encrypted data while locked, decrypts after unlock", async () => {
    ...
    expect(await getData<{ secret: string }>("incidents-z")).toEqual({
      secret: "later",
    });
  });
  ...
  it("stores plaintext (disclosed) when no vault is configured", async () => {
    ...
    expect(await getData<{ theme: string }>("prefs")).toEqual({
      theme: "dark",
    });
  });
});
