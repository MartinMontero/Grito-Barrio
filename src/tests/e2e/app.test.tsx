...
describe('App: boot & shell', () => {
  it('renders the home screen inside the app shell (no vault => no lock screen)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/Grito|Protocolo|Apoyo Comunitario/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('navigates to Protocolos and shows protocol content', async () => {
    renderApp('/protocols')
    await waitFor(() => {
      expect(screen.getAllByText(/Respuesta Inmediata|Prevención de Desalojos|Protocolos/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('shows the legal section', async () => {
    renderApp('/legal')
    await waitFor(() => {
      expect(screen.getAllByText(/Derechos|Legal|Ley de Vivienda/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('shows the resources/contacts section', async () => {
    renderApp('/resources')
    await waitFor(() => {
      expect(screen.getAllByText(/911|Recursos|Emergencia|Contactos/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('renders settings', async () => {
    renderApp('/settings')
    await waitFor(() => {
      expect(screen.getAllByText(/Configuración|Ajustes|Seguridad/i).length).toBeGreaterThan(0)
    })
  })
