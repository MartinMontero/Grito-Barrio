...
beforeEach(() => {
  vault.lock()
  localStorage.clear()
  useProtocoloStore.setState({ incidents: [], activeIncidentId: null, incidentHistory: [], checklists: {} })
})
...
describe('emergency journey (real app)', () => {
  it('an active incident drives the emergency dashboard', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText(/Centro|Crítico|crítico|Incidente|Emergencia/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('the checklist screen renders the store-backed items for the incident', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText(/Confirmar la alerta|Verificar seguridad de la escena|protocolo de respuesta/i).length
      ).toBeGreaterThan(0)
    })
  ...
  it('the evidence screen renders capture options', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/Foto|Video|Audio|Nota|Evidencia|Documentar/i).length).toBeGreaterThan(0)
    })
  })
  ...
  it('legal triage renders for the active incident', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/Triage|Legal|Ocupante|Desalojo|Continuar|Siguiente/i).length).toBeGreaterThan(0)
    })
  })
})
