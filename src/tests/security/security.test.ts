...
describe("security: vault-backed authentication", () => {
  it("hasPassword reflects vault existence; verifyPassword detects real vs duress vs wrong", async () => {
    ...
    vault.lock();
    expect(await securityManager.verifyPassword("wrong")).toEqual({
      valid: false,
      isDuress: false,
    });
    vault.lock();
    expect(await securityManager.verifyPassword("real-pass-123")).toEqual({
      valid: true,
      isDuress: false,
    });
    vault.lock();
    expect(await securityManager.verifyPassword("duress-pass-123")).toEqual({
      valid: true,
      isDuress: true,
    });
  });
  ...
  it("unlocking with the duress password schedules a durable panic wipe", async () => {
    ...
    expect(
      localStorage.getItem("protocolo_cdmx_panic_wipe_state"),
    ).toBeTruthy();
    securityManager.cancelWipe();
  });
});
...
describe("security: lockout after repeated failures", () => {
  it("throws once the configured max failed attempts is reached", async () => {
    ...
    await expect(
      securityManager.verifyPassword("nope-again"),
    ).rejects.toBeTruthy();
  });
});
