...
export function isEmergencyPhase(value: unknown): value is EmergencyPhase {
  const validPhases: EmergencyPhase[] = [
    "0-5min",
    "5-20min",
    "20-45min",
    "45-60min",
  ...
  return (
    typeof value === "string" && validPhases.includes(value as EmergencyPhase)
  );
}
...
export function isThreatLevel(value: unknown): value is ThreatLevel {
  ...
  return (
    typeof value === "string" && validLevels.includes(value as ThreatLevel)
  );
}
...
export { CONTACT_CATEGORIES, PRIORITY_LABELS } from "./contacts";
