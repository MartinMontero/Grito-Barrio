...
const TrainingDashboard = lazy(() =>
  import("@/components/features/TrainingDashboard").then((m) => ({
    default: m.TrainingDashboard,
  })),
);
const TrainingModuleViewer = lazy(() =>
  import("@/components/features/TrainingModule").then((m) => ({
    default: m.TrainingModuleViewer,
  })),
);
const CertificationTracker = lazy(() =>
  import("@/components/features/CertificationTracker").then((m) => ({
    default: m.CertificationTracker,
  })),
);
const ScenarioSimulator = lazy(() =>
  import("@/components/features/ScenarioSimulator").then((m) => ({
    default: m.ScenarioSimulator,
  })),
);
const ContactDirectory = lazy(() =>
  import("@/components/features/ContactDirectory").then((m) => ({
    default: m.ContactDirectory,
  })),
);
const ContactTree = lazy(() =>
  import("@/components/features/ContactTree").then((m) => ({
    default: m.ContactTree,
  })),
);
const SafePointsMap = lazy(() =>
  import("@/components/features/SafePointsMap").then((m) => ({
    default: m.SafePointsMap,
  })),
);
const SafePointDetail = lazy(() =>
  import("@/components/features/SafePointDetail").then((m) => ({
    default: m.SafePointDetail,
  })),
);
const SupplyChecklist = lazy(() =>
  import("@/components/features/SupplyChecklist").then((m) => ({
    default: m.SupplyChecklist,
  })),
);
const FormFiller = lazy(() =>
  import("@/components/features/FormFiller").then((m) => ({
    default: m.FormFiller,
  })),
);
const FormViewer = lazy(() =>
  import("@/components/features/FormViewer").then((m) => ({
    default: m.FormViewer,
  })),
);
const MessageTemplates = lazy(() =>
  import("@/components/features/MessageTemplates").then((m) => ({
    default: m.MessageTemplates,
  })),
);
const QuickDial = lazy(() =>
  import("@/components/features/QuickDial").then((m) => ({
    default: m.QuickDial,
  })),
);
const RoleSwitcher = lazy(() =>
  import("@/components/features/RoleSwitcher").then((m) => ({
    default: m.RoleSwitcher,
  })),
);
const SecuritySettings = lazy(() =>
  import("@/components/features/SecuritySettings").then((m) => ({
    default: m.SecuritySettings,
  })),
);
const DuressMode = lazy(() =>
  import("@/components/features/DuressMode").then((m) => ({
    default: m.DuressMode,
  })),
);
...
const TRAINING_MODULES: TrainingModuleData[] = [
  {
    ...
    description:
      "Proteger - Avisar - Socorrer: principios básicos de respuesta",
    ...
    lessons: [
      {
        ...
        checkpoint: {
          id: "mod-1-l1-cp",
          description:
            "Recuerda: la seguridad de las personas siempre es la prioridad número uno.",
          isCompleted: false,
        },
      },
      {
...
const FORM_TEMPLATE_LIST: FormTemplate[] = [
  {
    ...
    legalNotice:
      "Este documento es confidencial y puede usarse como evidencia. Verifique la exactitud de los datos antes de firmar.",
    sections: [
      {
        id: "alerta",
        ...
        fields: [
          ...
          },
          {
            ...
            placeholder: "Hotline, red comunitaria...",
          },
        ],
      },
      {
        ...
        fields: [
          ...
          { id: "alcaldia", type: "text", label: "Alcaldía", required: true },
          {
            ...
            type: "textarea",
            label: "Referencias",
            placeholder: "Edificio, piso, señas...",
          },
        ],
      },
      {
  ...
  },
  {
    ...
    lastUpdated: new Date().toISOString(),
    legalNotice:
      "El testimonio se recoge con el consentimiento de la persona y se mantiene bajo confidencialidad.",
    sections: [
      {
        id: "entrevista",
...
function ContactTreeRoute() {
  const dial = (contactId: string) => {
    ...
    const phone =
      contact?.phones.find((p) => p.primary)?.number ||
      contact?.phones[0]?.number;
    if (phone) window.location.href = `tel:${phone.replace(/[^0-9+]/g, "")}`;
  };
  return (
    <ContactTree contacts={DIRECTORY_CONTACTS} onCall={dial} onMessage={dial} />
  );
}
...
function FormFillerRoute() {
  ...
  const [templateId, setTemplateId] = useState<string>(
    FORM_TEMPLATE_LIST[0].id,
  );
  const template =
    FORM_TEMPLATE_LIST.find((t) => t.id === templateId) ||
    FORM_TEMPLATE_LIST[0];
...
function FormViewerRoute() {
  ...
  const formData: FormData = filledForms.get(template.id) || {
    id: `form-${template.id}`,
...
function RoleSwitcherRoute() {
  ...
  const role: TeamRole = isTeamRole(currentUser?.role)
    ? currentUser!.role
    : "leader";
