...
const ProtocolDetailPage = lazy(() =>
  import("@/components/features/ProtocolDetailPage").then((m) => ({
    default: m.ProtocolDetailPage,
  })),
);
const LegalResourceDetailPage = lazy(() =>
  import("@/components/features/LegalResourceDetailPage").then((m) => ({
    default: m.LegalResourceDetailPage,
  })),
);
const LegalTriageWizard = lazy(() =>
  import("@/components/features/LegalTriageWizard").then((m) => ({
    default: m.LegalTriageWizard,
  })),
);
const SettingsPage = lazy(() =>
  import("@/components/features/SettingsPage").then((m) => ({
    default: m.SettingsPage,
  })),
);
const EmergencyModal = lazy(() =>
  import("@/components/features/EmergencyModal").then((m) => ({
    default: m.EmergencyModal,
  })),
);
const EmergencyDashboard = lazy(() =>
  import("@/components/features/EmergencyDashboard").then((m) => ({
    default: m.EmergencyDashboard,
  })),
);
const EmergencyChecklist = lazy(() =>
  import("@/components/features/EmergencyChecklist").then((m) => ({
    default: m.EmergencyChecklist,
  })),
);
const PASProtocolGuide = lazy(() =>
  import("@/components/features/PASProtocolGuide").then((m) => ({
    default: m.PASProtocolGuide,
  })),
);
const EvidenceCollection = lazy(() =>
  import("@/components/features/EvidenceCollection").then((m) => ({
    default: m.EvidenceCollection,
  })),
);
const RoleSelector = lazy(() =>
  import("@/components/features/RoleSelector").then((m) => ({
    default: m.RoleSelector,
  })),
);
const RoleDashboard = lazy(() =>
  import("@/components/features/RoleDashboard").then((m) => ({
...
function ProtocolsRoute() {
  const navigate = useNavigate();
  return (
    <ProtocolsPage onProtocolSelect={(p) => navigate(`/protocols/${p.id}`)} />
  );
}
...
function RoleSelectorRoute() {
  ...
  return (
    <RoleSelector
      onRoleSelect={(roles: TeamRole[]) =>
        roles[0] && navigate(`/roles/${roles[0]}`)
      }
  ...
  return (
    <RoleDashboard
      ...
      recentActivity={incidents.slice(0, 5).map((inc) => ({
        ...
        actor:
          (inc as { reporterPseudonym?: string }).reporterPseudonym ||
          "Operador",
      }))}
...
function App() {
  ...
  useEffect(() => {
    ...
    const off = onUnlockChange((unlocked) =>
      setLocked(!unlocked && getVaultState() === "locked"),
    );
    return off;
  }, [ensureLocalUser]);
