...
export function useOffline(): UseOfflineReturn {
  ...
  const syncWhenOnline = useCallback(<T,>(operation: () => Promise<T>): Promise<T | null> => {
  ...
  const wasRecentlyOffline = useCallback((withinMs: number = 60000): boolean => {
  ...
  }, [handleOnline, handleOffline, handleConnectionChange, updateNetworkInfo, checkConnection, state.isOnline])
...
export function useOfflineQueue() {
  ...
  useEffect(() => {
    if (isOnline && queue.current.length > 0) {
      console.log(`[useOfflineQueue] Executing ${queue.current.length} queued operations`)
  ...
  const enqueue = useCallback((operation: () => void, immediateIfOnline = true) => {
