...
export function useIDBList<T>(
  ...
  const { enabled = true, refreshInterval, onError, ...queryOptions } = options;
  ...
  const update = useCallback(
    async (id: string | number, item: T): Promise<boolean> => {
      try {
        const result = await db.put(storeName, item);
...
export function usePersistentState<T>(
  ...
  const [value, setStoredValue, removeValue] = useLocalStorage<T>(
    key,
    defaultValue,
  );
